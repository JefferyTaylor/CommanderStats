...
`config.js` is not committed (see `.gitignore`). Deploy uses **GitHub Actions** so secrets become `config.js` only in the published artifact.

1. In the GitHub repo, open **Settings → Secrets and variables → Actions → New repository secret** and add:
   - `SUPABASE_URL` — e.g. `https://xxxx.supabase.co`
   - `SUPABASE_ANON_KEY` — publishable / anon key from Supabase
   - (Optional) `SUPABASE_RECORDER_PIN` — same value you use locally in `config.js` for the lightweight PIN
2. **Settings → Pages**: under **Build and deployment**, set **Source** to **GitHub Actions** (not “Deploy from a branch”).
3. Merge or push to **`main`**. The workflow **Deploy GitHub Pages** runs and publishes the site.
4. After the first run, open the site URL shown on the workflow run (and on **Settings → Pages**). It will look like:
   - `https://<your-username>.github.io/<repo-name>/`

If the workflow fails on “Create Pages site”, enable Pages once in repo settings and re-run the workflow.
